...
def _download_ftp_file(ftp, name, dest, overwrite):
    dest=dest.split('/')[-1]
...
def download_ftp_tree(ftp, path, destination, pattern=None, overwrite=False, guess_by_extension=True):
    path = path.lstrip("/")
...
def timerCheckFTP(interval):
    ...
    for station in stations:
        ...
        download_ftp_tree(ftp, remote_dir, local_dir, overwrite=False, guess_by_extension=True)
